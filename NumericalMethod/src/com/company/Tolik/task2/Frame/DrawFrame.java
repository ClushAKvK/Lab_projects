...
import static com.company.Tolik.task2.Main.Main.maxX;
import static com.company.Tolik.task2.Main.Main.minX;
...
public class DrawFrame {
    ...
    public static void draw(Dot[] dots) {
        ...
        for(int i = 0; i < k; i++) {
            series.add(
                    dots[i].x,
                    Newton.run(dots[i].x, dots)
            );
        }

        for (double i = minX; i < maxX; i+=0.01) {
//            series2.add(i, Math.abs(i * i - 2 * i));
            series2.add(i, Math.sin(i));
            //series2.add(i, Math.sin(i * i));
        }
